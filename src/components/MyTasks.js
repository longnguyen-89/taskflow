...
export default function MyTasks({ tasks, members, userId, onOpenTask, profileName }) {
  ...
  function relation(t) {
    ...
    if (isAssignee) return { l: 'Được giao', c: '#2D5A3D', bg: '#e8f5ee' };
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <div>
        <h2 className="font-display font-bold text-lg" style={{ color: '#2D5A3D' }}>Công việc tôi</h2>
      ...
      {/* Filter */}
      <div className="flex p-0.5 rounded-lg w-fit" style={{ background: '#f0ebe4' }}>
        {[
          { id: 'all', l: 'Tất cả' },
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {groups.weekByDay.map(([day, list]) => (
            <Section key={'w' + day} title={`📅 ${fmtDateLong(day + 'T12:00:00Z')}`} color="#2D5A3D" list={list} />
          ))}
